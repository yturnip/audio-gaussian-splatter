...
{
    GaussianManifold DomeBranchingGenerator::generate(const GeneratorConfig &config) const
    {
        ...
        {
            const float y = numHubs > 1 ? (float) i / (float) (numHubs - 1) : 0.0f;;
            const float radius = std::sqrt(std::max(0.0f, 1.0f - y * y));
            const float theta = phi * i;

            const float hx = radius * std::sin (theta);
            const float hy = y;
            const float hz = radius * std::cos (theta);
